...
// Anti-eco v4: 1 aviso por comando. Mostra quando o texto da resposta muda;
// resposta repetida so aparece 1 vez logo apos um comando enviado (consome a janela).
const loadResultMemory = () => {
  try {
    const raw = JSON.parse(localStorage.getItem('lsLastCommandResults') || '{}');
    Object.keys(raw).forEach((k) => {
      if (typeof raw[k] === 'string') {
        raw[k] = { result: raw[k], shownAt: 0 };
      }
    });
    return raw;
  } catch {
...
const filterRepeatedCommandResults = (events) => {
  const memory = loadResultMemory();
  let expecting = Date.now() < Number(localStorage.getItem('lsExpectResultUntil') || 0);
  let changed = false;
  const out = events.filter((event) => {
    if (event.type !== 'commandResult') {
      return true;
    }
    const result = event.attributes ? event.attributes.result : '';
    const prev = memory[event.deviceId] || { result: null, shownAt: 0 };
    const newText = prev.result !== result;
    const show = newText || expecting;
    if (show) {
      memory[event.deviceId] = { result, shownAt: Date.now() };
      changed = true;
      expecting = false;
      localStorage.setItem('lsExpectResultUntil', '0');
    }
    return show;
  });
  if (changed) {
    localStorage.setItem('lsLastCommandResults', JSON.stringify(memory));
  }
  return out;
};
...
const SocketController = () => {
  ...
  // --- Vigia de bloqueio no navegador ---
  // O rastreador nao manda o alarme lock/unlock e o evento inserido pelo
  // conector nao vem pelo websocket. Mas a POSICAO chega em tempo real e
  // traz o atributo 'blocked'. Detectamos a virada aqui: aviso + som.
  const blockStateRef = useRef(new Map());

  const alertBlock = useCallback(
    (deviceId, blocked) => {
      ...
      const preset = soundsMap[blocked ? 'lock' : 'unlock'] || (blocked ? 'sireneForte' : 'rising');
      if (preset && preset !== 'none') {
        playPreset(preset);
      }
      setNotifications((prev) => [
        ...prev.slice(-2),
        {
          id: `block-${deviceId}-${Date.now()}`,
          message: `${name}: veiculo ${label}`,
          show: true,
        },
      ]);
    },
    [devices, lsAlertSounds],
  );
  const alertBlockRef = useRef(alertBlock);
  alertBlockRef.current = alertBlock;

  const checkBlockChanges = useCallback((positions) => {
    positions.forEach((p) => {
      if (!p || !p.attributes || p.attributes.blocked === undefined) {
        return;
      }
      const cur = Boolean(p.attributes.blocked);
      const prev = blockStateRef.current.get(p.deviceId);
      blockStateRef.current.set(p.deviceId, cur);
      if (prev !== undefined && prev !== cur) {
        alertBlockRef.current(p.deviceId, cur);
      }
    });
  }, []);
  const checkBlockChangesRef = useRef(checkBlockChanges);
  checkBlockChangesRef.current = checkBlockChanges;
    ...
    events
      ...
      .forEach((event) => {
        ...
        const timer = setTimeout(() => {
          pendingResultsRef.current.delete(event.deviceId);
          const shown = filterRepeatedCommandResults([event]);
          if (shown.length) {
            deliverRef.current(shown);
          }
        }, 2500);
  ...
  const connectSocket = useCallback(() => {
    ...
    socket.onmessage = (event) => {
      ...
      if (data.positions) {
        ...
        checkBlockChangesRef.current(data.positions);
      }
      if (data.events) {
