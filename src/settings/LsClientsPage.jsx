...
import {
  ...
  Chip, Fab, Typography, Button, Dialog, DialogTitle, DialogContent,
  DialogContentText, DialogActions, Radio, RadioGroup, FormControlLabel,
  CircularProgress,
} from '@mui/material';
...
const LsClientsPage = () => {
  ...
  const toggleSuspend = useCatchCallback(async (user) => {
    ...
    <PageLayout menu={<SettingsMenu />} breadcrumbs={['settingsTitle']}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 16px 0' }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <Button size="small" variant="outlined" onClick={() => setImportOpen(true)}>Importar CSV</Button>
          <Button size="small" variant="outlined" onClick={handleExport} disabled={!items || !items.length}>Exportar CSV</Button>
        </div>
      </div>
      <Table size="small">
        <TableHead>
        ...
        <TableBody>
            ...
            <TableRow key={user.id}>
              ...
              <TableCell align="right">
                ...
                <Tooltip title="Editar">
                  <IconButton size="small" onClick={() => navigate(`/settings/client/${user.id}`)}>
          ...
          {items && !items.length && (
            <TableRow>
              <TableCell colSpan={6}>Nenhum cliente ainda — clique no + ou importe o CSV da Infinity.</TableCell>
            </TableRow>
          )}
        </TableBody>
      </Table>
      <Fab color="primary" style={{ position: 'fixed', bottom: 24, right: 24 }} onClick={() => navigate('/settings/client')}>
        <AddIcon />
      </Fab>
      <Dialog open={importOpen} onClose={() => !importing && setImportOpen(false)} fullWidth maxWidth="xs">
        ...
        <DialogContent>
          ...
          <Typography variant="body2" style={{ fontWeight: 600, margin: '14px 0 4px' }}>Perfil para todos deste arquivo</Typography>
          <RadioGroup value={importPerfil} onChange={(e) => setImportPerfil(e.target.value)}>
            ...
            <FormControlLabel value="alertas" control={<Radio size="small" />} label="Ver + alertas" />
            <FormControlLabel value="bloqueio" control={<Radio size="small" />} label="Ver + alertas + bloqueio" />
          </RadioGroup>
          <Typography variant="caption" color="textSecondary">
            Senha inicial = telefone do cliente. Veículos você vincula depois, na edição de cada cliente.
          </Typography>
        </DialogContent>
        <DialogActions>
        ...
        <DialogContent>
          <DialogContentText component="div" style={{ fontSize: 14 }}>
            ...
            {importResult && importResult.erros.length > 0 && (
              <div style={{ marginTop: 8, fontSize: 12.5 }}>{`Linhas com problema (sem e-mail válido): ${importResult.erros.join(', ')}`}</div>
            )}
          </DialogContentText>
        </DialogContent>
        <DialogActions>
          <Button variant="contained" onClick={() => setImportResult(null)}>Fechar</Button>
        </DialogActions>
      </Dialog>
      <RemoveDialog
        ...
        endpoint="users"
        itemId={removing}
        onResult={(done) => { setRemoving(null); if (done) load(); }}
      />
    </PageLayout>
  );
};
